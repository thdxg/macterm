...
A few Ghostty keys steer Macterm's own behavior and have no Settings equivalent: `tab-inherit-working-directory` and `split-inherit-working-directory` decide whether a new tab or split starts in the focused pane's directory (`true`) or at the project root (`false`); `mouse-scroll-multiplier` sets scrollback speed for trackpads (`precision:`) and mouse wheels (`discrete:`), and applies inside vim and less too; `macos-shortcuts` gates [Shortcuts](/docs/shortcuts). Macterm ships two of these with a different default from Ghostty — new tabs start at the project root (`tab-inherit-working-directory = false`) and a wheel notch scrolls one row rather than three (`mouse-scroll-multiplier = precision:1,discrete:1`) — and your config overrides either like any other key.
