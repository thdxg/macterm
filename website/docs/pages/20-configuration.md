...
A few Ghostty keys steer Macterm's own behavior and have no Settings equivalent: `tab-inherit-working-directory` and `split-inherit-working-directory` decide whether a new tab or split starts in the focused pane's directory (`true`) or at the project root (`false`), and `macos-shortcuts` gates [Shortcuts](/docs/shortcuts). Macterm ships one of these with a different default from Ghostty — new tabs start at the project root (`tab-inherit-working-directory = false`) — and your config overrides it like any other key. Scrolling is Ghostty's own, so `mouse-scroll-multiplier` (with its `precision:` and `discrete:` halves) tunes trackpad and wheel speed exactly as it does in Ghostty.
...
**Settings → Animations**, under Appearance, holds the motion settings. Smooth scrolling and split animations are on; the two cursor effects are off until you turn them on.

- **Smooth scrolling** — trackpad scrolling moves scrollback by pixels instead of whole rows, including during momentum, and dragging the scrollbar does the same instead of jumping a row at a time. Only scrollback scrolls this way: programs that draw their own screen (editors, pagers, `less`) still scroll by rows, because that is how they repaint. Mouse wheels keep row-by-row scrolling, and `mouse-scroll-multiplier` tunes the speed of both as in Ghostty.
...
- **Animate splits** — Hyprland-style slide animations for the split layout. A new pane slides in from its own edge of the split while its neighbor makes room (a side-by-side split from the right, a stacked split from the bottom), a closing pane slides back out through that edge, zooming grows the pane over the layout, and a pane dragged to another spot travels there. Dividers go with it: closing a pane in the middle of a row or column merges the two dividers either side of it into one, each travelling the same distance to meet, and splitting peels a new divider off the one it grew from. Dragging a divider is never animated. The animation is off automatically when Reduce Motion is on in System Settings.
