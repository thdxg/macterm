...
The panel is centered by default. To move it, enable **Allow moving the panel** in **Settings → Quick Terminal**: a grab handle appears along the panel's top edge, and dragging it moves the panel like a title bar would. The position is remembered, so the panel reopens where you left it (always fully on screen, even if the screen changed underneath it).
