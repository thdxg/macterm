...
A global terminal accessible from anywhere with a hotkey — <kbd>⌃`</kbd> by default. It drops down over your current space, then gets out of your way when you're done. The quick terminal is scratch space: its sessions end on every quit, and its size and hotkey are configurable in **Macterm → Settings**. Besides the hotkey, **Quick Terminal** sits in the View menu and **Toggle Quick Terminal** in the menu you get by right-clicking Macterm's Dock icon — that one leaves whatever app you're in focused, just as the hotkey does.
