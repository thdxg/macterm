...
To open a directory as a project, start typing a path (anything beginning with `/` or `~`). The palette switches to path mode and autocompletes directories as you go. A remote spec like `devbox:~/dev/api` works too — it offers to add (or switch to) that [remote project](/docs/remote-projects).
