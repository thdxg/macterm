...
`mise run e2e` builds the Debug app and runs a pytest suite (`e2e/`, venv-bootstrapped by `scripts/e2e.sh`) that launches the real GUI app hermetically via `MactermHarness` and asserts through the bundled `macterm` CLI — `pane dump`/`pane inspect`/`pane list` read libghostty's own live state, so a passing I/O test proves the whole stack (socket → AppState → libghostty → pty → shell → renderer cells). Debug configuration on purpose: correctness is under test, it shares CI's Debug DerivedData cache with the unit-test job, and the debug-only verbs (`pane resize`) stay available. CI runs it as the `e2e` job in `checks.yml`; failures upload a `e2e-diagnostics` artifact (screenshot, streamed unified log, per-pane text — captured by `e2e/conftest.py`'s autouse fixtures).

Suite conventions: ONE app instance per session (launch costs ~15–30s), so tests own their blast radius — take the `fresh_tab` fixture for a disposable tab (an autouse janitor force-closes leftovers), never mutate the initial project pane. Every wait is a deadline poll (`_harness.wait_for`), never a fixed sleep. Commands typed into panes must parse in ANY login shell (CI is zsh; dev machines may run nushell/fish) — write them as `/bin/sh -c "…"` with a single-quote-free script, the same trick as the benchmark workload and `RemoteSpawn`. Target panes explicitly (`--session`/`--pane`); `pane run` goes through `MactermHarness.pane_run`, which places connection flags *before* the command (its passthrough capture would otherwise swallow a trailing `--socket` into the typed text and fall back to discovering a real Macterm's socket).
