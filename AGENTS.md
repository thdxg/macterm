...
**Ghostty keys Macterm applies to its own behavior** have no Settings UI: `macos-shortcuts` (App Intents), `tab-inherit-working-directory` / `split-inherit-working-directory` (`false` = the project directory — ghostty's "default working directory" in a project-based terminal; `AppState.newTabInheritsWorkingDirectory` is the injectable read), `mouse-scroll-multiplier` (`MouseScrollMultiplier`, fed to Macterm's scrollback accumulator per device class so one key drives both it and libghostty's own alt-screen path; raw text, because the key is a struct with no C getter), `macos-hidden`, `macos-icon`. **Where Macterm's default departs from ghostty's, the departure is a line in `macterm-defaults.conf`** (`MactermConfig.defaultsBody`, pinned by `MactermConfigTests`: `tab-inherit-working-directory = false`, `mouse-scroll-multiplier = precision:1,discrete:1`), never a `Preferences` fallback — so the user overrides it the way they override any other key.
...
- `Macterm/Config/` — `MactermConfig`, `GhosttyConfigSource`, `GhosttyConfigText`, `ShellIntegrationFeatures`, `GhosttyColorSpace`, `MacosHidden`, `MouseScrollMultiplier`.
