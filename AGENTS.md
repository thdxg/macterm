...
**Wire format (`RemoteSpawn`, hard-won against a real host):** every remote command ships as `sh -c '<single-quote-free script>'`. `sh -c`, NOT `sh -lc` — the login flag is unportable (older dash, common as `/bin/sh`, rejects `-l` and silently drops to an interactive shell that ignores the command). The single-quote-free script is the one form bash/zsh/fish/nu all tokenize identically before POSIX `sh` takes over. **PATH:** sshd runs commands with a bare non-interactive PATH, so the script harvests PATH from `/etc/profile`/`~/.profile` — sourced **inside a command-substitution subshell, never inline**. Profiles are arbitrary code: a real host's `~/.profile` ended in `exec zsh`, which (sourced inline under our `>/dev/null` silencing) replaced the script wholesale — the exec'd zsh inherited stdout/stderr→/dev/null, yielding a pane with a visible prompt (ZLE writes to /dev/tty), visible keystrokes (kernel pty echo), and invisible command output, zmx never attached. In a subshell, an `exec`/`exit`/abort only kills the subshell; the harvested PATH is adopted when non-empty, then the fallback dir list is appended (`~/bin`, `~/.local/bin`, `~/.cargo/bin`, `/usr/local/bin`, `/opt/homebrew/bin`). **`zmxPath` escape hatch:** PATH resolution still can't cover every host (network-mounted home, exotic `/bin/sh`, PATH set only in a non-POSIX shell config), so a `Project.zmxPath` (optional, in the sheet + `projects.json` + `ProjectFile`) is used verbatim, bypassing PATH entirely and skipping the presence guard. **Failure visibility:** when zmx isn't found (PATH mode) or the `cd` fails, the script prints a `macterm:` diagnostic and drops to `${SHELL:-/bin/sh}` instead of exiting — a bare exit would fire `closeSurface` and vanish the pane with no clue why. **TERM is forced to `xterm-256color`** for remote sessions: the forwarded `xterm-ghostty` breaks TUIs on hosts without its terminfo, and `xterm-256color` is the one universally-safe value. (An earlier claim that zmx renders blind under `TERM=xterm-ghostty` came from an experiment confounded by the profile-hijack bug above — unverified; if disproven, a conditional fallback could return.)

Constraints, all deliberate: zmx must be preinstalled on the host (set `zmxPath` if it's not on the non-interactive PATH); port/identity/ControlMaster/IPv6 come from `~/.ssh/config` aliases, never Macterm flags; the orphan reaper stays local-only (zero-client `macterm-*` sessions on a shared host legitimately belong to other machines); local-cwd features (Replace Project Path with Current Dir, live `run:`/`shell:` capture on Save Layout) are disabled for remote panes; declared layout `cwd`/`~` resolve remote-side as pure strings (`LayoutBuilder.resolveCwd`). Tested against a Docker aarch64 host (nushell login shell, dash `/bin/sh`, zmx in `~/bin`, broken `~/.profile`) — see `scratchpad` harness in PR #138.
