...
- **`bell-features = attention` has two halves.** `GHOSTTY_ACTION_RING_BELL` handles the app-level ones (beep, `bell-audio-path`, `NSApp.requestUserAttention`) in `GhosttyCallbacks.ringBell` and additionally flags the ringing pane (`Pane.ringBell`); `title`/`border` are per-tab UI Macterm doesn't implement. The Dock badge is the second half: `BellBadge.label` is the pure count → label rule (nil without `attention` or with nothing ringing, capped `99+`) and `BellBadge.apply` is the one AppKit write, glued by `AppState.syncDockBadge`. It counts **tabs**, not ghostty's windows — a tab is what the user goes and looks at — and is DERIVED, never written from the ring, which is what lets a config reload dropping `attention` clear a badge already up with no code path that knows about clearing. Acknowledgment follows Macterm's existing "looking at the active tab" verdict rather than ghostty's next-keypress rule, so the badge can't disagree with the sidebar: a bell in the active tab of the active app is seen the instant it rings, and everything else clears the way the completion dot does (tab selection, pane focus or interaction, the app coming forward, surface teardown). Each flag flip posts `.terminalBellStateDidChange`, so clearing re-derives for free.
...
Right-clicking the Dock tile offers **New Window**, **New Tab**, **New Project…**, **Toggle Quick Terminal** — Ghostty's set, built by `AppDelegate.applicationDockMenu` (`Macterm/App/DockMenu.swift`; `@NSApplicationDelegateAdaptor` does forward this one, verified live). Items are `AppCommand`s run through `AppCommand.action(in:)`, so the Dock is a fourth renderer of that list rather than a second set of handlers; the menu is rebuilt per right-click with `autoenablesItems` off (enabled ⇔ non-nil action), and is nil until `MainWindow` hands the delegate its state objects. What the Dock adds is `DockMenu.Preparation`: a pick arrives from outside the app and does not activate it, so New Tab / New Project… call `showWindow()` first (the hidden last window, or the #241 no-window state), New Window activates and lets the command make its own, and Toggle Quick Terminal prepares **nothing** — the panel is non-activating by design. The action is resolved *after* preparing, since fronting a window is what makes `activeProjectID` the right project. `DockMenu.Preparation` is now the shared **"invoked from outside the app"** table — App Intents run through the same `AppDelegate.performExternalCommand` rather than deciding again, which is why it carries `.toggleCommandPalette` (unreachable from the Dock menu itself).

### App Intents (Shortcuts, Spotlight)

Thirteen intents in `Macterm/Intents/`, mirroring Ghostty's surface (`macos/Sources/Features/App Intents/`) in Macterm's model: **New Project**, **New Tab**, **Run Command in Pane**, **Send Key**, **Get Pane Contents**, **Get Pane Details**, **Focus Project/Tab/Pane**, **Close Tab**, **Toggle Quick Terminal**, **Open Command Palette**, **Invoke Keybind**. User-facing grammar in `website/docs/pages/85-shortcuts.md`. `AppIntents.framework` is an SDK dependency in `project.yml`; the build's `appintentsmetadataprocessor` writes `Contents/Resources/Metadata.appintents/extract.actionsdata`.

- **Translate-and-delegate only**, exactly like `ControlHandler`: resolve what an entity names, then call the same `AppState`/`ProjectStore` methods the UI calls. `ControlHandler`'s own resolution is deliberately *not* shared — it speaks selector strings (name / 1-based index / UUID) while an entity's identity is exact, so `IntentTargets` does UUID/session lookups and the CLI is untouched.
- **`MactermIntentHost`** is the only way an intent reaches the app (the system instantiates intents, so nothing can be handed in). `AppDelegate.installResponders` attaches it, and readiness waits on `AppState.performWhenRestored` — running a shortcut is what *launches* the app, so `perform()` can arrive before the restore. The wait is a bounded sleep-poll (`readinessTimeout`), because a launch that fronts no window may never install responders (#241) and a hang would wedge the shortcut with no error.
- **Entities are tokens, not handles.** `MactermPaneEntity.id` is the zmx **session name**, not `Pane.id`: pane UUIDs are fresh on every restore, so a saved shortcut keyed on one would break at the first relaunch *silently* (the pane it names is visibly still there). Tab and project ids are persisted, so those are fine. Every lookup can answer `.notFound`.
- **The user's ghostty `macos-shortcuts`** (`ask`/`allow`/`deny`, default `ask`) gates every `perform()`, via `IntentPermissionGate` in front of the readiness wait. It is read live off the loaded config (`GhosttyApp.shortcutsAccess` → `ShortcutsAccess.resolve`, the `MacosHidden` shape; the raw values are ghostty's tag names) and has no Settings UI. `ask` remembers the answer for the run only (`.allow` is the standing grant; Ghostty instead persists an Allow forever under one UserDefaults key and remembers a Deny not at all). Under `MACTERM_BENCHMARK=1` it resolves to allow without a modal — the harnesses drive the app with nobody at the keyboard, the same reason the notification prompt is skipped — while an explicit `deny` still denies.
- **`MactermKeybind` restates `HotkeyAction`'s cases and titles by necessity**: `appintentsmetadataprocessor` reads `caseDisplayRepresentations` out of the source and rejects a computed dictionary ("must be a dictionary"). `MactermIntentsTests` pins the case set against `HotkeyAction.allCases` and every label against its `AppCommand.title`, so a new keybind fails a test rather than quietly missing from the picker. Same constraint applies to any future `AppEnum`.
- **Statics must be `let`** under `SWIFT_STRICT_CONCURRENCY: complete` (`title`, `description`, `defaultQuery`, `supportedModes`), and `supportedModes` is `#if compiler(>=6.2)`-gated like Ghostty's. `IntentFile`'s `supportedContentTypes:` is macOS 15 only — use `supportedTypeIdentifiers:` rather than gating an intent away from the 14 floor.
- **`linkd` indexes an app's intents only after it has been launched from a stable location.** A bundle in `/private/tmp` fails registration outright (`Could not create application record … -10814`); one copied to `~/Applications` but never run logs `is not trusted for binding, skipping`. Launch it once and the audit proceeds (`Checking … extract.actionsdata` → `Collating specified bundles`). Not a signing problem — reproduced identically ad-hoc and with a release-shaped self-signed cert. Verify with `log show --predicate 'process == "linkd"'`.
...
**Ghostty keys Macterm applies to its own behavior** have no Settings UI: `macos-shortcuts` (App Intents), `tab-inherit-working-directory` / `split-inherit-working-directory` (`false` = the project directory — ghostty's "default working directory" in a project-based terminal; `AppState.newTabInheritsWorkingDirectory` is the injectable read), `macos-hidden`, `macos-icon`. **Where Macterm's default departs from ghostty's, the departure is a line in `macterm-defaults.conf`** (`MactermConfig.defaultsBody`, pinned by `MactermConfigTests`: `tab-inherit-working-directory = false`), never a `Preferences` fallback — so the user overrides it the way they override any other key. **Scrolling is libghostty's on every path**: `GhosttyTerminalNSView.scrollWheel` forwards events exactly as Ghostty.app does (2× precise deltas, momentum phase) and `SurfaceScrollView` only draws the scroller and turns scroller drags into `scroll_to_row`. The iTerm2-style row accumulator from #102 (with its Scroll speed slider) was removed so `mouse-scroll-multiplier` means the same thing here as in Ghostty; don't reintroduce a Macterm-side wheel path.

Overrides Macterm must lock (plus the opt-in `smooth-scroll`/`custom-shader`/`cursor-opacity` lines from `MactermConfig.Experiments`, see Experimental settings): `background-default-transparent = true` (fork patch — the renderer skips the default background so `WindowAppearance` composites translucency itself), `background-opacity = <Preferences.windowOpacity>` (the real value, so the user's `background-opacity-cells` works), `background-blur = 0` (Macterm calls the CGS blur SPI), `env = GHOSTTY_BIN_DIR=<bundle>/Contents/Resources/ssh-bridge` (holds our `ghostty` shim relaying `+ssh` to `macterm ssh`; kept out of any dir on the pane PATH), and `shell-integration-features` re-emitted with the user's flags plus `no-path` (`ShellIntegrationFeatures.overrideValue`, #75 — the key can't be written bare). Macterm UI state lives in `Preferences` and never touches this pipeline.
...
A pane for features that work but haven't earned a permanent home: every toggle defaults **off**, does nothing to a user who never opens the pane, and is expected to graduate or be removed. Four live there today: two pure ghostty shaders, one fork patch, and one piece of Macterm chrome.

- **Smooth scrolling** is fork patch 0005 (`.github/downstream/0005-smooth-scroll.patch`) behind the fork key `smooth-scroll` (default off), which the toggle writes into the overrides via `MactermConfig.Experiments`. libghostty's `scrollCallback` already accumulates precise trackpad deltas in pixels and commits whole rows; with the key on, the patch keeps the sub-row remainder on `Screen.viewport_pixel_offset`, has `RenderState` carry one extra row (the one being revealed; `rows_above` says which end) and shifts the grid by a `scroll_offset` uniform, clipping to the visible grid sized from `grid_size × cell_size` — **never from `grid_padding`'s bottom/right**, which hold the leftover measured at the last resize and go stale after the incubator→pane growth. Any row-level viewport move goes through `Screen.scroll`, which zeroes the remainder. Macterm has no part in the routing (every wheel event reaches libghostty untouched, #393), which is exactly why the gate is a ghostty key and not a `Preferences` read. Only scrollback scrolls smoothly; alt-screen programs repaint their own rows.
- **Animate splits** (`Preferences.animatedSplits`) swaps the recursive `SplitTreeView` for `AnimatedSplitView`, which renders the tab's tree **flat**: `SplitLayout` (pure, tested) resolves every pane and divider to an absolute rect, and one ZStack keyed by pane ID lays them out, so any tree change is frames moving on stable identities and SwiftUI animates it — the recursive view can't, because re-nesting a leaf changes its structural identity. `SplitRootView` is the switch; both call sites (`WorkspaceView`, the quick terminal) go through it. Rules, each verified live: **a platform view moves only by frame** (`.scaleEffect` and the `.scale`/`.move` transitions leave an NSView where it is), so a new pane grows out of its split's seam from a zero-length frame, edge-locked to the shrinking sibling; **nothing crosses another pane** (backgrounds are transparent, so overlap is text through text, and an empty new pane is invisible anyway); **a closing pane is a ghost** — `Pane.destroySurface` snapshots the last frame (`Pane.closingSnapshot`, only while the toggle is on) *without* a background fill, because an opaque ghost flashed to full opacity in a translucent window — clipped to the strip the sibling has not yet reclaimed; direction is the pane's **own outer edge** of the split (right/bottom pane slides right/down, left/top pane left/up); the animation is keyed to structure plus zoom, never ratios, so a divider drag lands immediately; each arriving or ghost animation starts from its own `onAppear`, the same frame as the sibling's, since a one-turn deferral visibly desynchronised the seam. Zoom keeps the hidden tiles mounted at opacity 0 (`GhosttyTerminalNSView.hiddenInLayout` sleeps their renderers) but removes the dividers, whose grab bands are NSViews that would catch drags through the zoomed pane. Every animation frame resizes the moving surfaces, the same path a divider drag takes; Reduce Motion disables the animation.
- **Smooth cursor** and **Cursor trail** are bundled custom shaders (`Macterm/Resources/shaders/`, a folder reference — the one non-downloaded thing under `Resources/`, un-ignored in `.gitignore`). `MactermConfig.Experiments` appends them to the overrides as `custom-shader` lines (the key is repeatable and the overrides load last, so the user's own shaders stay and run first), trail before glide so the streak renders beneath the drawn cursor. The glide shader *is* the focused cursor: it needs `cursor-opacity = 0`, the one user-visible ghostty key a Macterm setting overrides (libghostty applies it only while focused, so unfocused panes keep the native hollow cursor). The glyph under a block cursor is recovered from `iChannel0` by its cursor-text color, since ghostty still paints it even with the cursor invisible. **The bundled files are templates**: `ExperimentShaders.install` writes them to `<App Support>/shaders/` with a `#define` header (`MACTERM_LINEAR_BLENDING`, `MACTERM_DISPLAY_P3`) read from the user's stated `alpha-blending` / `window-colorspace`, because a custom shader writes straight into ghostty's framebuffer — gamma-encoded Display P3 under macOS's default `native` blending, config colors taken as P3 under `display-p3` — and its uniforms don't say which; `mactermEncode` mirrors `load_color` in ghostty's `shaders.metal`, where the community shaders' `sRGBToLinear` assumes a linear target and draws a visibly darker cursor. `iCursorVisible` is 0 while no cursor glyph is drawn (scrolled out of the viewport, blink-off) — fork patch 0006, since upstream leaves the position uniforms stale and a self-drawn cursor would linger at the old cell. A ghostty shader can shift pixels but cannot invent the row entering at an edge, which is why smooth *scrolling* could not be a shader.
...
- `Macterm/App/` — `AppState` (+ `AppState+PinnedTabs`, `AppState+FirstRun`, `AppState+QuickTerminal`), `WindowState`, `Preferences`, `Hotkeys`/`KeyRouter`/`Responders`/`KeybindPassthrough`, `AppCommand`+`AppCommandActions`+`AppCommandMenu` (single source of truth for user-invokable actions — palette, menus and Settings render from `AppCommand.allCases`), `FocusRestoration`, `Updater` (Sparkle), `AppInfo`, `FirstRunSeed`, `Tutorial`, `FinderServices`+`OpenFolder`, `DockMenu`, `BellBadge` (+ `AppState+BellBadge`), `BenchmarkControl`, `ExceptionReporting`, `EnvironmentSetup`, `Notifications`/`NotificationHandler`, `PollCadence`, `RecencyStack`, `TabIndexChord`.
...
- `Macterm/Config/` — `MactermConfig` (+ `Experiments`), `ExperimentShaders`, `GhosttyConfigSource`, `GhosttyConfigText`, `ShellIntegrationFeatures`, `GhosttyColorSpace`, `MacosHidden`.
- `Macterm/Resources/shaders/` — the bundled cursor-effect shaders (`cursor_glide.glsl`, `cursor_trail.glsl`); everything else under `Resources/` is a setup.sh download.
...
- `Pane` IDs are fresh on every restore; session identity (`sessionID`/`sessionName`, stored verbatim) is what reattaches. A declared layout is applied only by an explicit Apply Layout (palette, menu, Settings row, `layout apply`); selecting a project or relaunching never reads it — the snapshot is what restores, and a project without one gets the default workspace.
...
2. If bindable, add the `HotkeyAction` to `Hotkeys.swift` with its default, **and the matching `MactermKeybind` case + label** — the App Intents picker restates them and `MactermIntentsTests` fails until it matches.
...
- **Local sessions don't survive reboot**; remote ones do. Every pane — workspace, pinned, quick terminal — persists via zmx and reattaches after a quit.
