...
- **`Preferences.shortcutsAccess`** (`ask`/`allow`/`deny`, default `ask`) gates every `perform()`, via `IntentPermissionGate` in front of the readiness wait. It mirrors Ghostty's `macos-shortcuts` key (same values, same default) but is a **`Preferences` value, not a ghostty key** — the intents create projects and type into shells, none of which libghostty has a stake in, and inventing a key ghostty doesn't define would break "the user is the source of truth for every ghostty setting". `ask` remembers the answer for the run only (`.allow` is the standing grant; Ghostty instead persists an Allow forever under one UserDefaults key and remembers a Deny not at all). Under `MACTERM_BENCHMARK=1` it resolves to allow without a modal — the harnesses drive the app with nobody at the keyboard, the same reason the notification prompt is skipped — while an explicit `deny` still denies.
...
Overrides Macterm must lock (plus the opt-in `custom-shader`/`cursor-opacity` lines from `MactermConfig.CursorEffects`, see Experimental settings): `background-default-transparent = true` (fork patch — the renderer skips the default background so `WindowAppearance` composites translucency itself), `background-opacity = <Preferences.windowOpacity>` (the real value, so the user's `background-opacity-cells` works), `background-blur = 0` (Macterm calls the CGS blur SPI), `env = GHOSTTY_BIN_DIR=<bundle>/Contents/Resources/ssh-bridge` (holds our `ghostty` shim relaying `+ssh` to `macterm ssh`; kept out of any dir on the pane PATH), and `shell-integration-features` re-emitted with the user's flags plus `no-path` (`ShellIntegrationFeatures.overrideValue`, #75 — the key can't be written bare). Macterm UI state lives in `Preferences` and never touches this pipeline.
...
- **Smooth scrolling** is a routing decision on the Macterm side plus a renderer patch in the fork (`.github/downstream/0005-smooth-scroll.patch`). libghostty's `scrollCallback` already accumulates precise trackpad deltas in pixels and commits whole rows; the patch keeps the sub-row remainder on `Screen.viewport_pixel_offset`, has `RenderState` carry one extra row (the one being revealed; `rows_above` says which end) and shifts the grid by a `scroll_offset` uniform, clipping to the visible grid sized from `grid_size × cell_size` — **never from `grid_padding`'s bottom/right**, which hold the leftover measured at the last resize and go stale after the incubator→pane growth. Any row-level viewport move goes through `Screen.scroll`, which zeroes the remainder. On the Macterm side, `SurfaceScrollView.handleSurfaceScrollWheel` normally claims every wheel event and quantizes it through the iTerm2 accumulator (`sendScrollToRow`), which would discard exactly the remainder — so with the toggle on it **declines precise events** and `GhosttyTerminalNSView.scrollWheel` forwards them, scaled by the same `terminalScrollSpeed` the accumulator uses. Off, nothing reaches the renderer with a remainder, so no config key is needed. Only scrollback scrolls smoothly; alt-screen programs repaint their own rows.
- **Smooth cursor** and **Cursor trail** are bundled custom shaders (`Macterm/Resources/shaders/`, a folder reference — the one non-downloaded thing under `Resources/`, un-ignored in `.gitignore`). `MactermConfig.CursorEffects` appends them to the overrides as `custom-shader` lines (the key is repeatable and the overrides load last, so the user's own shaders stay and run first), trail before glide so the streak renders beneath the drawn cursor. The glide shader *is* the focused cursor: it needs `cursor-opacity = 0`, the one user-visible ghostty key a Macterm setting overrides (libghostty applies it only while focused, so unfocused panes keep the native hollow cursor). The glyph under a block cursor is recovered from `iChannel0` by its cursor-text color, since ghostty still paints it even with the cursor invisible. A ghostty shader can shift pixels but cannot invent the row entering at an edge, which is why smooth *scrolling* could not be a shader.
...
- `Macterm/Config/` — `MactermConfig` (+ `CursorEffects`), `GhosttyConfigSource`, `ShellIntegrationFeatures`, `GhosttyColorSpace`, `MacosHidden`.
