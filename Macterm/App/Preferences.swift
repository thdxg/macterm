...
final class Preferences {
    ...
    /// keeps running in the background. Default off — session persistence (shells
    /// survive quit and reattach on relaunch) is the point, so quit detaches
    /// rather than terminates. Macterm-side only; never touches the ghostty
    /// config pipeline.
