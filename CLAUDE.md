...
| File                     | Purpose                                                                            |
| ------------------------ | ---------------------------------------------------------------------------------- |
| `GhosttyApp.swift`       | libghostty init, config, tick loop, color queries; resolves `GHOSTTY_RESOURCES_DIR`/`TERMINFO` |
| `GhosttyResources.swift` | Pure `GhosttyResourceResolver` — picks the resources dir + terminfo path (testable selection logic) |
...
libghostty reads two things at runtime via `GHOSTTY_RESOURCES_DIR`: `shell-integration/` and `terminfo/` (both mandatory), plus `themes/` when a named theme is used. Macterm ships all three in its own app bundle (`Contents/Resources/`) so it runs with no Ghostty.app install. The tarball is downloaded by `setup.sh` from the `thdxg/ghostty` release (`ghostty-resources.tar.gz`) and folder-referenced into the bundle via `project.yml`. Nothing under `Macterm/Resources/` is committed — it's all gitignored and regenerated by `setup.sh`.

`GhosttyApp.resolveResources()` points `GHOSTTY_RESOURCES_DIR` at the bundle (falling back to an installed `Ghostty.app` if the bundle is missing resources, e.g. an unprepared dev checkout). The selection logic is factored into the pure, unit-tested `GhosttyResourceResolver` in `GhosttyResources.swift`.

Two non-obvious terminfo details (the regression behind issues #39/#40, where 1.13.3 pointed `GHOSTTY_RESOURCES_DIR` at the bundle but shipped no terminfo, breaking `TERM=xterm-ghostty` and key input):

- **TERMINFO is set explicitly.** libghostty derives `TERMINFO = dirname(GHOSTTY_RESOURCES_DIR)/terminfo`, but Macterm's `GHOSTTY_RESOURCES_DIR` is `Contents/Resources` (themes/shell-integration sit directly there, not under a `ghostty/` subdir), so that derivation would point outside Resources. So `resolveResources()` sets `TERMINFO` to `<bundle>/terminfo` directly — but only for our own bundle, since the Ghostty.app fallback derivation is already correct.
...
| `Ghostty/GhosttyResourceResolverTests.swift`                                                                                | Resource dir selection + TERMINFO derivation (`GhosttyResourceResolver`)             |
| `Ghostty/BundledResourcesTests.swift`                                                                                       | Bundle ships terminfo (`78/xterm-ghostty`), shell-integration, themes (#39/#40 guard) |
