...
@MainActor
struct AnimationShadersTests {
    // MARK: - Encoding: the framebuffer contract read off the user's config

    @Test
    func no_config_is_macos_defaults() {
        #expect(AnimationShaders.Encoding.from(userConfigText: nil) == .macOSDefault)
        #expect(AnimationShaders.Encoding.from(userConfigText: "font-size = 16\n") == .macOSDefault)
        #expect(AnimationShaders.Encoding.macOSDefault == .init(linearBlending: false, displayP3: false))
    }

    @Test
    func linear_and_linear_corrected_both_mean_a_linear_framebuffer() {
        #expect(AnimationShaders.Encoding.from(userConfigText: "alpha-blending = linear\n").linearBlending)
        #expect(AnimationShaders.Encoding.from(userConfigText: "alpha-blending = linear-corrected\n").linearBlending)
        #expect(!AnimationShaders.Encoding.from(userConfigText: "alpha-blending = native\n").linearBlending)
    }

    @Test
    func display_p3_is_read_and_last_value_wins() {
        let text = "window-colorspace = display-p3\nalpha-blending = linear\nalpha-blending = native\n"
        let encoding = AnimationShaders.Encoding.from(userConfigText: text)
    ...
    func unknown_values_fall_back_like_ghosttys_parser_rejecting_them() {
        let encoding = AnimationShaders.Encoding.from(userConfigText: "alpha-blending = bogus\nwindow-colorspace = rec2020\n")
    ...
    func header_carries_both_defines_and_the_template_is_untouched() {
        let template = "// body\nvoid mainImage() {}\n"
        let rendered = AnimationShaders.render(
            template: template,
